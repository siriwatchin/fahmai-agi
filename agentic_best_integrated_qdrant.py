...
PROJECT_ROOT = env_path("PROJECT_ROOT", "/project/zz992000-zdevb")
WORK = env_path("WORK_DIR", PROJECT_ROOT / "fahmai_outputs")
SRC = env_path("SRC_DIR", PROJECT_ROOT / "scamper_house")
DATA = env_path("DATA_DIR", SRC / "fah-mai-the-finale-enterprise-data-agentic-showdown")
QUESTIONS_CSV = env_path("QUESTIONS_CSV", SRC / "questions.csv")
QUESTIONS_XLSX = env_path("QUESTIONS_XLSX", SRC / "question.xlsx")
MODEL = env_path("QWEN_MODEL_PATH", SRC / "qwen35/models/Qwen2.5-7B-Instruct")
TOKEN_LOG = []
...
_RETRIEVED_CHUNK_LIMIT = int(os.getenv("RETRIEVED_CHUNK_LIMIT", "1200"))
_STRIP_PATTERNS = [
    re.compile(
        r"(?i)(ignore|disregard|forget|override)\s+"
        r"(all\s+)?(previous|above|prior|your|the)?\s*"
        r"(instructions?|rules?|prompts?|guidelines?|constraints?|system\s+prompt)"
    ),
    re.compile(
        r"(?i)(you\s+are\s+now|act\s+as|pretend\s+(you\s+are|to\s+be)|roleplay\s+as)\s+"
        r"(an?\s+)?(unrestricted|jailbreak|DAN|evil|uncensored|unfiltered|GPT-?\d*)"
    ),
    re.compile(r"<\|im_start\|>(?!assistant|user|system)"),
    re.compile(r"<\|im_end\|>"),
    re.compile(r"\[INST\]|\[/INST\]|\[SYS\]|\[/SYS\]"),
    re.compile(r"(?i)<<SYS>>|<</SYS>>"),
    re.compile(r"(?i)(new|updated?|revised?)\s+system\s+prompt\s*:"),
    re.compile(r"(?im)^SYSTEM\s*:\s*(you\s+are|your\s+new\s+role|from\s+now)"),
    re.compile(r"(?i)reveal\s+(your\s+)?(system\s+prompt|instructions?|rules?|constraints?)"),
]


def sanitize_retrieved_text(text, chunk_limit=_RETRIEVED_CHUNK_LIMIT):
    ...
    if len(text) > chunk_limit:
        cutoff = text.rfind(" ", 0, chunk_limit)
        if cutoff < chunk_limit * 0.8:
            cutoff = chunk_limit
        text = text[:cutoff]
    return text, was_injected


def sanitize_search_results(rows, chunk_limit=_RETRIEVED_CHUNK_LIMIT):
    sanitized = []
    injection_count = 0
    for row in rows or []:
        item = dict(row)
        clean, injected = sanitize_retrieved_text(item.get("text", ""), chunk_limit=chunk_limit)
        item["text"] = clean
        if injected:
            item["injection_redacted"] = True
            injection_count += 1
        sanitized.append(item)
    return sanitized, injection_count
...
def answer_one(sqltool, retriever, qdrant_retriever, tok, model, qid, q):
    docs_raw = retriever.search(q, 8)
    docs, doc_injection_count = sanitize_search_results(docs_raw)
    schemas = sqltool.schema_search(q, 10)

    ans, obs = hard_sql_answer(sqltool, qid, q, docs, schemas)
    if ans:
        if isinstance(obs, dict):
            obs["injection_redactions"] = {"document_search": doc_injection_count, "qdrant_search": 0}
        return sanitize_answer(ans), obs

    qdrant_docs_raw = qdrant_retriever.search(q, 8) if qdrant_retriever else []
    qdrant_docs, qdrant_injection_count = sanitize_search_results(qdrant_docs_raw)
    if doc_injection_count or qdrant_injection_count:
        print(
            "injection_redacted:",
            qid,
            "document_search=",
...
""".strip()
    return gen(tok, model, prompt, qid=qid, stage="final_answer", max_new_tokens=160), {
        ...
        "injection_redactions": {
            "document_search": doc_injection_count,
            "qdrant_search": qdrant_injection_count,
        },
    }
